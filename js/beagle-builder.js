            ...
            if (Math.floor(Date.now()) - timeStored > 3) {
                ...
                if (typeof components !== 'undefined') {
                    for (var i = 0; i < components.length; i++) {
                        ...
                        var newEle = $("<div id='" + components[i].replace(".html", "") + "' class='beagle-component-premade-item'><div class='beagle-component-block-overlay'><h4>" + components[i].replace(".html", "") + "</h4></div></div>");
...
//        });
        ...
        $(document).on("click", ".beagle-tool", function () {
            if ($(this).attr("tool") != "component-tool") {
...
//            $('meta[name="viewport"]').prop('content', '');
    ...
    //                        $("<a href='www.google.com'/>").insertAfter(selectedEle);
        ...
        $(document).on("click", ".beagle-component-premade-item", function () {
            if (Math.floor(Date.now()) - timeStored > 3) {
                timeStored = Math.floor(Date.now());
                if ($(".beagle-selected").length > 0) {
                    var selectedEle = $(".beagle-selected");
                    var thisEle = $(this);
                    var appendingHTML = $(thisEle.find(".beagle-component-premade-inner").html());
