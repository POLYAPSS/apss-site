    ...
    $.ajax({
        ...
        for (var i = 0; i < response.length; i++) {
            if (response[i].download_url.includes(".png") || response[i].download_url.includes(".jpg") || response[i].download_url.includes(".jpeg") || response[i].download_url.includes(".svg") || response[i].download_url.includes(".gif")) {
                $("#beagle-image-select-area, #beagle-background-image-select-area").append('<div class="beagle-image-select-gallery-image"><img class="beagle-image-select-target" src="' + response[i].download_url + '"></div>');   
...
function updateMediaGallery() {
    $("#beagle-image-select-area, #beagle-background-image-select-area").empty();
    $("#main-html img").each(function () {
        var imageSrc = $(this).attr("src");
        if (imageSrc && !allImageSrc.includes(imageSrc)) {
            $("#beagle-image-select-area, #beagle-background-image-select-area").append('<div class="beagle-image-select-gallery-image"><img class="beagle-image-select-target" src="' + imageSrc + '"></div>');
        }
    });
    if (typeof nameWithOwner !== 'undefined' && typeof uploadMediaToken !== 'undefined' && typeof mediaFolder !== 'undefined') {
        getMedia();   
    }
    $("#beagle-image-select-area, #beagle-background-image-select-area").append('<div style="clear: both;"></div>');
}
...
//                        selectElement($(".beagle-selected"));
        ...
        $(document).on("click", ".beagle-image-select-target", function () {
            if (toolSelected != "design-tool") {
                ...
                $("#beagle-background-image-source-edit").val($(this).attr("src"));
                $(".beagle-selected").css("background-image", "url(" + $("#beagle-background-image-source-edit").val() + ")");
            }
        });
